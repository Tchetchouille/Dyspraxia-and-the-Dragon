...
func _on_exp_dragon_death() -> void:
	if $"../../Main".global_dyspraxia == true:
		var dyspraxia_win_scene = load("res://Scenes/UI/win_1_story.tscn").instantiate()
		get_tree().root.add_child(dyspraxia_win_scene)
		$"..".queue_free()
	else:
		var dyspraxia_win_scene = load("res://Scenes/UI/win_2_story.tscn").instantiate()
		get_tree().root.add_child(dyspraxia_win_scene)
		$"..".queue_free()

func _on_exp_celestia_character_death() -> void:
	if get_node_or_null("../MainUi/VBoxContainer/HBoxContainer/MiddleMargin/Clock") == null:
		if $"../../Main".global_dyspraxia == true:
			var dyspraxia_game_over_scene = load("res://Scenes/UI/game_over_dyspraxia_story.tscn").instantiate()
			get_tree().root.add_child(dyspraxia_game_over_scene)
			$"..".queue_free()
		else:
			var dyspraxia_game_over_scene = load("res://Scenes/UI/game_over_celestia_story.tscn").instantiate()
			get_tree().root.add_child(dyspraxia_game_over_scene)
			$"..".queue_free()
	else:
		$"../../Main".global_score = $"../MainUi/VBoxContainer/HBoxContainer/DragonHealth/Score".score
